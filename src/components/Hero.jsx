...
const Hero = () => (
  ...
  >
    ...
    {['top-8 left-8 border-t border-l', 'top-8 right-8 border-t border-r',
      ...
      .map((cls, i) => (
        <div key={i} className={`absolute w-4 h-4 ${cls} border-white/10 pointer-events-none`} />
    ))}
    ...
    <div
      ...
      style={{ fontSize: 'clamp(100px, 25vw, 340px)', letterSpacing: '-0.05em' }}
    >
      UK
    </div>
    ...
    <motion.div variants={item} className="mb-8 z-10">
      <img src={Logo} alt="Logo" className="w-8 h-8 md:w-10 md:h-10 object-contain mx-auto opacity-50" />
    </motion.div>
    ...
    <motion.div variants={item} className="z-10">
      <Magnetic factor={0.2}>
        ...
        >
          ...
          <span className="relative z-10 text-primary transition-transform duration-300 group-hover:translate-x-1">→</span>
          <span
    ...
    >
      <span className="text-[7px] font-mono tracking-[0.5em] uppercase">Scroll</span>
      <motion.div
        animate={{ y: [0, 6, 0] }}
